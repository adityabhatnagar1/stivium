...
import { isPathOrDescendant, rebasePath } from "../utils/editor";
...
export function useWorkspaceActions({
  ...
  const getTargetDirectory = () => {
    ...
    return selectedNode.node.isDirectory
  ...
  const closeTabsUnderDeletedPath = (
    deletedPath: string,
    isDirectory: boolean,
  ) => {
    setTabs((prev) => {
      const survivors = prev.filter((tab) =>
        isDirectory
          ? !isPathOrDescendant(tab.path, deletedPath)
      ...
      if (survivors.length !== prev.length) {
        setActiveTabPath((prevActive) => {
          ...
          const activeWasClosed = isDirectory
            ? isPathOrDescendant(prevActive, deletedPath)
            : prevActive === deletedPath;

          if (!activeWasClosed) return prevActive;

          return survivors.length > 0
            ? survivors[survivors.length - 1].path
            : null;
        });
      }
  ...
  const submitNameDialog = async () => {
    ...
    if (!selectedNode || name === selectedNode.node.name) {
      return;
    }

    await withFsAction(async () => {
      const oldPath = selectedNode.node.path;
      const newPath = oldPath.replace(/[^\\/]+$/, name);

      await window.api.renamePath(oldPath, name);

      if (selectedNode.node.isDirectory) {
        setTabs((prev) =>
          prev.map((tab) => {
            if (!isPathOrDescendant(tab.path, oldPath)) {
              return tab;
            }
        ...
        if (activeTabPath && isPathOrDescendant(activeTabPath, oldPath)) {
          setActiveTabPath(rebasePath(activeTabPath, oldPath, newPath));
        }
      } else {
  ...
  const runContextAction = async (
    ...
    if (action === "new-file") {
      return openNameDialog("create-file");
    }

    if (action === "new-folder") {
      return openNameDialog("create-folder");
    }
  ...
  const runWorkspaceAction = async (
      ...
      setConfirmDialog({
        ...
        onConfirm: async () => {
          await withFsAction(async () => {
            ...
            closeTabsUnderDeletedPath(node.path, node.isDirectory);
    ...
    if (action === "copy") {
      setClipboard({
        sourcePath: selectedNode.node.path,
        cut: false,
      });
